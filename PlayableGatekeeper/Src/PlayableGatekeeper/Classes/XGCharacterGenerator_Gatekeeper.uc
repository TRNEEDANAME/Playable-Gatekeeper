class XGCharacterGenerator_Gatekeeper extends XGCharacterGenerator
	dependson(X2StrategyGameRulesetDataStructures);

var localized string GatekeeperFirstName;
var localized string GatekeeperLastNamePrefix;
...
function GenerateName( int iGender, name CountryName, out string strFirst, out string strLast, optional int iRace = -1 )
{
	...
	strFirst = GatekeeperFirstName;
	strLast = GatekeeperLastNamePrefix;
	SparkIndex = 1;
	bUseRandomNum = true;
	...
	if(History != none)
	{
		...
		if(XComHQ != none)
		{
			...
			for(idx = 0; idx < XComHQ.Crew.Length; idx++)
			{
				...
				if(UnitState != none && UnitState.GetMyTemplateName() == 'PA_Gatekeeper')
				{
					SparkIndex++;
				}
			}
